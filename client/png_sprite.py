import pygame

bongo_sprite = [
    pygame.image.load('../../Assets/bongo_sprite/bongoup.png'),  # 0
    pygame.image.load('../../Assets/bongo_sprite/bongodown.png'),  # 1
    pygame.image.load('../../Assets/bongo_sprite/bongomeowup.png'),  # 2
    pygame.image.load('../../Assets/bongo_sprite/bongomeowdown.png'),  # 3
    pygame.image.load('../../Assets/bongo_sprite/bongoDR.png'),  # 4
    pygame.image.load('../../Assets/bongo_sprite/bongomeowDR.png'),  # 5
    pygame.image.load('../../Assets/bongo_sprite/bongoDL.png'),  # 6
    pygame.image.load('../../Assets/bongo_sprite/bongomeowDL.png'),  # 7
    pygame.image.load('../../Assets/bongo_sprite/bongofin.png'),  # 8
    pygame.image.load('../../Assets/bongo_sprite/sadbongo1.png'),  # 9
    pygame.image.load('../../Assets/bongo_sprite/angrybongo.png'),  # 10
    pygame.image.load('../../Assets/bongo_sprite/madbongobar.png'),  # 11
    pygame.image.load('../../Assets/bongo_sprite/bongodownsay.png')  # 12
]

bg_sprite = [
    pygame.image.load('../../Assets/bg/skybg.png'),
    pygame.image.load('../../Assets/bg/keyboard.png'),
    pygame.image.load('../../Assets/bg/lobby.jpg'),
    pygame.image.load('../../Assets/bg/insertnamebg.png'),
    pygame.image.load('../../Assets/bg/textbox8.png'),
    pygame.image.load('../../Assets/bg/waiting.png'),
    pygame.image.load('../../Assets/bg/hatf.png'),
    pygame.image.load('../../Assets/bg/losescreen.jpg'),
    pygame.image.load('../../Assets/bg/txtbox.png'),
    pygame.image.load('../../Assets/bg/winbackground.jpg'),
    pygame.image.load('../../Assets/bg/winscreen.png'),
    pygame.image.load('../../Assets/bg/textbox2.png'),
    pygame.image.load('../../Assets/bg/drawscreen.jpg'),
    pygame.image.load('../../Assets/bg/dbox.png'),
    pygame.image.load('../../Assets/bg/finalbg.png'), #14
    pygame.image.load('../../Assets/bg/black_screen1.png'), #15
    pygame.image.load('../../Assets/bg/black_screen2.png'), #16
]

button_sprite = [
    pygame.image.load('../../Assets/text_surface/black_text_box.png'),
    pygame.image.load('../../Assets/bg/replay.png'),  # 1
    pygame.image.load('../../Assets/bg/exitnew1.png')  # 2
]

addi_sprite = [
    pygame.image.load('../../Assets/addi/santahat.png'),
    pygame.image.load('../../Assets/addi/tophat.png')
]

boom_sprite = [
    pygame.image.load('../../Assets/boom_sprite/b1.png'),
    pygame.image.load('../../Assets/boom_sprite/b2.png'),
    pygame.image.load('../../Assets/boom_sprite/b3.png'),
    pygame.image.load('../../Assets/boom_sprite/b4.png'),
    pygame.image.load('../../Assets/boom_sprite/b5.png'),
    pygame.image.load('../../Assets/boom_sprite/b6.png'),
    pygame.image.load('../../Assets/boom_sprite/b7.png'),
    pygame.image.load('../../Assets/boom_sprite/b8.png'),
    pygame.image.load('../../Assets/boom_sprite/b9.png'),
    pygame.image.load('../../Assets/boom_sprite/b10.png'),
    pygame.image.load('../../Assets/boom_sprite/b11.png'),
    pygame.image.load('../../Assets/boom_sprite/b12.png')
]
